<form
  x-data="form('authcred_reset_password')"
...
<template x-if="$store.form.step === 2">
  <form
    x-data="form('authcred_reset_password_confirm')"
    class="my-2 space-y-4 max-w-sm"
    @submit.prevent="dispatch"
  >
    <div x-show="success && message" class="p-2 text-green-700 border rounded border-green-900/10 bg-green-50" x-cloak>
    ...
    <div>
      <label for="code" class="relative z-0 w-full">
        <input type="text" id="code" name="code" class="block pt-4 pb-1 px-2 border-gray-200 text-sm rounded bg-transparent border appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
    ...
    <div class="flex items-center justify-between">
      ...
      <input type="hidden" name="nonce" value="<?= wp_create_nonce('authcred_reset_password_confirm') ?>">
...
<template x-if="$store.form.step === 3">
  <form
    x-data="form('authcred_reset_password_new')"
    class="my-2 space-y-4 max-w-sm"
    @submit.prevent="dispatch"
  >
    <div x-show="success && message" class="p-2 text-green-700 border rounded border-green-900/10 bg-green-50" x-cloak>
    ...
    <div class="flex items-center justify-between">
      ...
      <input type="hidden" name="nonce" value="<?= wp_create_nonce('authcred_reset_password_new') ?>">
